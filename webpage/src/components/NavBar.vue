<template>
  <v-app-bar app clipped-right color="primary" dark :clipped-left="$vuetify.breakpoint.lgAndUp">
    ...
    <v-menu offset-y transition="slide-y-transition" bottom :close-on-content-click="false">
      ...
      <v-card>
        <v-list>
          ...
          <v-list-item>
            <v-select
              :items="Object.keys($i18n.messages).map(l => ({value:l, text:$i18n.messages[l].lang}))"
              v-model="$i18n.locale"
              :label="$t('language')"
            ></v-select>
          </v-list-item>
